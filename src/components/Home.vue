...
export default {
  ...
  computed: {
    filterData() {
      const vm = this;
      if (vm.searchText) {
        vm.$store.dispatch("PModule/filterP", vm.searchText);
        return vm.$store.state.PModule.filterProducts;
      }
      return vm.$store.state.PModule.products;
    },
  ...
  methods: {
    ...
    addtoCart(id, qty = 1) {
      this.$store.dispatch("CModule/addtoCart", { id, qty });
    },
    getUnique() {
      const vm = this;
      vm.$store.dispatch("PModule/getUnique");
    },
  },
  created() {
    this.getProducts();
